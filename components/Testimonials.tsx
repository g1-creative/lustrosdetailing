      ...
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className="group bg-luxury-dark p-8 rounded-xl border border-gray-800 
                         hover:border-luxury-gold transition-all duration-500 
                         card-hover-effect relative overflow-hidden"
            >
              {/* Quote Icon Background */}
              <div className="absolute top-4 right-4 text-luxury-gold/10 group-hover:text-luxury-gold/20 transition-colors duration-500">
              ...
              <div className="flex gap-1 mb-4 relative z-10">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <svg
                    key={i}
                    className="w-5 h-5 text-luxury-gold transform hover:scale-125 transition-transform duration-300"
                    fill="currentColor"
                    viewBox="0 0 20 20"
              ...
              {/* Testimonial Text */}
              <p className="text-gray-300 group-hover:text-white mb-6 text-lg leading-relaxed transition-colors duration-300 relative z-10 italic">
                "{testimonial.text}"
              </p>

              {/* Author Info */}
              <div className="border-t border-gray-800 group-hover:border-luxury-gold/30 pt-4 transition-colors duration-300 relative z-10">
                <div className="font-bold text-white mb-1 group-hover:text-luxury-gold transition-colors duration-300">{testimonial.name}</div>
                <div className="text-luxury-gold text-sm mb-1">{testimonial.vehicle}</div>
                {testimonial.location && (
                  <div className="text-gray-500 text-sm group-hover:text-gray-400 transition-colors duration-300 flex items-center gap-1">
              ...
              {/* Gradient Overlay */}
              <div className="absolute inset-0 bg-gradient-to-tr from-luxury-gold/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            </div>
          ))}
        </div>
